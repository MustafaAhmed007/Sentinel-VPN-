...
fn ps(script: &str) -> Result<()> {
    let out = Command::new("powershell.exe")
        ...
        .output()
        .context("launch PowerShell firewall operation")?;
...
fn quoted(s: &str) -> Result<String> {
    if s.contains('\'') || s.contains('\n') || s.contains('\r') || s.contains(';') {
        bail!("unsafe firewall parameter")
    }
    Ok(format!("'{}'", s))
}
...
impl FirewallController for WfpController {
    fn enter_lockdown(&mut self, vpn_endpoint: &str) -> Result<()> {
        ...
        ps(&format!(
            "Set-NetFirewallProfile -Profile Domain,Private,Public -Enabled True -DefaultInboundAction Block -DefaultOutboundAction Block; Remove-NetFirewallRule -Group {g}; New-NetFirewallRule -Name {e} -DisplayName {e} -Group {g} -Direction Outbound -Action Allow -Protocol UDP -RemoteAddress {ip} -RemotePort 1-65535 -Profile Any | Out-Null",
            g = quoted(GROUP)?,
            e = quoted(ENDPOINT_RULE)?,
            ip = ep
        ))?;
        self.mode = Some(FirewallMode::Lockdown {
            vpn_endpoint: endpoint.to_owned(),
        });
        Ok(())
    }

    fn allow_tunnel(&mut self) -> Result<()> {
        if !matches!(self.mode, Some(FirewallMode::Lockdown { .. })) {
            bail!("cannot allow tunnel before lockdown is active");
        }
        let alias = std::env::var("SENTINEL_WG_INTERFACE")
            .unwrap_or_else(|_| "Sentinel".to_string());
        let a = quoted(&alias)?;
        ps(&format!(
            "New-NetFirewallRule -Name {i} -DisplayName {i} -Group {g} -Direction Inbound -Action Allow -InterfaceAlias {a} -Profile Any | Out-Null; New-NetFirewallRule -Name {o} -DisplayName {o} -Group {g} -Direction Outbound -Action Allow -InterfaceAlias {a} -Profile Any | Out-Null",
            i = quoted(TUNNEL_IN_RULE)?,
            o = quoted(TUNNEL_OUT_RULE)?,
            g = quoted(GROUP)?,
            a = a
        ))?;
        Ok(())
