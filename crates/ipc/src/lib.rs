...
pub enum Command {
    GetStatus,
    Connect { profile: serde_json::Value, dns: serde_json::Value, endpoint: String },
    Disconnect { interface_name: String },
    Reconnect,
    RunDiagnostics,
    ListProfiles,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Envelope { pub request_id:String, pub command:Command }
#[derive(Debug, Clone, Serialize, Deserialize)]
...
#[cfg(test)]
mod tests { use super::*; #[test] fn round_trip(){let e=Envelope{request_id:"r1".into(),command:Command::Disconnect{interface_name:"Sentinel".into()}};let mut b=Vec::new();write_frame(&mut b,&e).unwrap();let got:Envelope=read_frame(&mut b.as_slice()).unwrap();assert_eq!(got.request_id,"r1");} }
